...
class TestCellTriangle(TestCase):

    def test_cell_triangle(self, verbose=False):
                    ...
                    for basis_1, coef_1 in zip(bases, coefs):
                        ...
                        for _i in range(euclidean_dimension):
                            for _j in range(euclidean_dimension):
                                ...
                                rtol = 1.0e-15
                                atol = 1.0e-15
                                print('MASS INTEGRAL CHECK | ORDER : {} | ELEM : {}'.format(polynomial_order,
                                                                                            element_type))
                                print("- QUADPY : {}".format(mass_integral_check))
                                print("- H2O : {}".format(mass_integral))
                                print(
                                    'STIFFNESS INTEGRAL CHECK | ORDER : {} | ELEM : {}'.format(polynomial_order,
                                                                                               element_type))
                                print("- QUADPY : {}".format(stif_integral_check))
                                print("- H2O : {}".format(stif_integral))
                                print(
                                    'ADVECTION INTEGRAL CHECK | ORDER : {} | ELEM : {}'.format(polynomial_order,
                                                                                               element_type))
