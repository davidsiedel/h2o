            ...
            while iteration < problem.number_of_iterations and not break_iteration:
                ...
                for _element_index, element in enumerate(problem.elements):
                    ...
                    def make_cell_procedure(break_iter: bool):
                        ...
                        # print("stab")
                        # print(element.stabilization_operator)
                        # print("K after stab")
                        # print(element_stiffness_matrix)
                            ...
                            if boundary_condition.boundary_type == BoundaryType.DISPLACEMENT:
                                for f_local, f_global in enumerate(element.faces_indices):
                                    if f_global in problem.mesh.faces_boundaries_connectivity[
                                        ...
                                        _v_face_current_displacement = np.zeros((_fk,), dtype=real)
                                        ...
                                        for _qf in range(face_quadrature_size):
                                            ...
                                            _v_face_current_displacement += (
                                                    w_coef * np.tensordot(face_displacement, _psi_k, axes = 1) * _psi_k
                                            )
                                            force_item += material.lagrange_parameter * (w_coef * v @ face_lagrange[:])
                                            # element_internal_forces[ _c0:_c1] += material.lagrange_parameter * w_coef * face_lagrange
                                            element_internal_forces[ _c0:_c1] += material.lagrange_parameter * w_coef * np.tensordot(face_lagrange, _psi_k, axes = 1) * _psi_k
                                            # rows_f = []
                                            # cols_f = []
                                            # data_f = []
                                            # for i_c, i_fill in enumerate(range(_l0, _l1)):
                                            #     for j_c, j_fill in enumerate(range(_r0, _r1)):
                                            #         data_f.append(material.lagrange_parameter * w_coef * np.tensordot(_psi_k, _psi_k, axes=0))
                                        ...
                                        # maxim = material.lagrange_parameter * _v_face_imposed_displacement * (1.0 / np.linalg.det(_m_psi_psi_face))
                                        maxim = 1.0 * material.lagrange_parameter * _v_face_imposed_displacement
                                        # print("np.linalg.det(_m_psi_psi_face) :", np.linalg.det(_m_psi_psi_face))
                                        if np.max(np.abs(maxim)) > external_forces_coefficient:
                                            external_forces_coefficient = np.max(np.abs(maxim))
                                            # _v_face_imposed_displacement += (
                                            #         _w_q_f * v * boundary_condition.function(time_step, _x_q_f)
                                            # )
                                        # ----- AVANT :
                                        residual[_l0:_l1] -= material.lagrange_parameter * (_v_face_current_displacement - _v_face_imposed_displacement)
                                        # residual[_l0:_l1] -= material.lagrange_parameter * (face_displacement - _v_face_imposed_displacement)
                                        ...
                                        for i_c, i_fill in enumerate(range(_l0, _l1)):
                                            for j_c, j_fill in enumerate(range(_r0, _r1)):
                                                data_f.append(material.lagrange_parameter * _m_psi_psi_face[i_c, j_c])
                                                rows_f.append(i_fill)
                                                cols_f.append(j_fill)
                                        tangent_matrix += coo_matrix((data_f, (rows_f, cols_f)), shape=(_constrained_system_size, _constrained_system_size))
                                        tangent_matrix += coo_matrix((data_f, (cols_f, rows_f)), shape=(_constrained_system_size, _constrained_system_size))
