...
class DisplacementFiniteElementField:
    ...
    def __init__(
        ...
        self.slide_faces_count = {}
        for bci, bc in enumerate(boundary_conditions):
            self.slide_faces_count[bc.boundary_name + "_" + bc.boundary_type.name] = 0
    ...
    def get_element_external_displacement_forces(self,
        ...
        for f_local, f_global in enumerate(element.faces_indices):
            if f_global in self.mesh.faces_boundaries_connectivity[boundary_condition.boundary_name]:
                process: bool = True
                if boundary_condition.boundary_type == BoundaryType.SLIDE and self.slide_faces_count[boundary_condition.boundary_name + "_" + boundary_condition.boundary_type.name] != 0:
                    process = False
                if process:
                    ...
                    self.slide_faces_count[boundary_condition.boundary_name + "_" + boundary_condition.boundary_type.name] += 1
        return
    ...
    def make_iteration(self,
        ...
        self.slide_faces_count = {}
        # for bci, bc in enumerate(self.boundary_conditions):
        #     self.slide_faces_count[bc.boundary_name + "_" + bc.boundary_type.name] = 0
        for boundary_condition in self.boundary_conditions:
            self.slide_faces_count[boundary_condition.boundary_name + "_" + boundary_condition.boundary_type.name] = 0
        ...
        if tolerance > -1.0:
            effective_tol: float = np.copy(tolerance)
        else:
            effective_tol: float = np.copy(self.tolerance)
        # effective_tol: float = self.tolerance
        if residual_evaluation < effective_tol:
            print("++++ DISPLACEMENT ITER : {} | RES_MAX : {:.6E} | TOLERANCE {:.6E} | CONVERGENCE | ITERATIONS : {}".format(
                str(iteration).zfill(4), residual_evaluation, effective_tol, iteration + 1))
